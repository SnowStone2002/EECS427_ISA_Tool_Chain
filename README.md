...
---
### 7. Compute-In-Memory 控制指令

该类指令用于控制和调用 CIM（计算存内）模块，实现与寄存器文件和数据存储器的互操作，支持权重存储、激活向量加载、计算执行以及结果读取。

**说明：**

- 所有 `R*` 表示寄存器名称。
- CIM 地址由寄存器提供，取其低 4 位作为实际地址，范围 0~15。
- DMEM 地址由寄存器提供，取其低 9 位。
...
- **STCR Rsrc, RCaddr**  
  将寄存器 `Rsrc` 中的 16 位数据写入到 CIM 单元，地址由寄存器 `RCaddr` 指定（取其低 4 位）。

- **STCM RCaddr, Raddr**  
  从 `DMEM[Raddr]` 读取 16 位数据，写入到 CIM 单元，地址由寄存器 `RCaddr` 指定。

- **LDCR Rdest, RCaddr**  
  从 CIM 单元（由寄存器 `RCaddr` 指定）的地址读取 16 位数据，写入到寄存器 `Rdest`。
...
- **CMPT Rsrc, Raddr**  
  执行 Compute-In-Memory 计算：
  1. 将寄存器 `Rsrc` 的 16 位数据作为激活向量；  
  2. 自动从 `Rsrc` 的低 2 位选择权重组（00→W0、01→W1 等）；  
  3. 进行点积运算并通过量化模块处理；  
  4. 将最终结果写入 `DMEM[Raddr]`（地址由寄存器 `Raddr` 指定）。

- **SETQ QNUM**  
  通过立即数 `QNUM`（4 位）设置量化位宽，范围 0~15。
...
```asm
# 将寄存器 R2 的值写入 CIM 单元，地址由寄存器 R1 指定
STCR    R2, R1

# 将 DMEM[R4] 的数据写入 CIM 单元，地址由寄存器 R3 指定
STCM    R3, R4

# 设置量化位宽为 8 位
SETQ    8

# 执行 Compute-In-Memory 计算：激活向量为 R4，结果写入 DMEM[R5]
CMPT    R4, R5

# 从 CIM 单元读取结果到寄存器 R6，地址由 R7 提供
LDCR    R6, R7
```
